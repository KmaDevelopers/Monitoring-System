...
class m111129_141922_someData extends CDbMigration
{
	public function up() {
		...
		for ($i = 0; $i < 50; $i++) {
			...
			$x = rand(0, 400);
			$y = rand(0, 400);
			$this->execute("insert into `Sensor` (`serverId`, `serial`,`name`,`position`,`path`) 
							values ({$sId}, 'serial{$i}', 'sensor{$i}','{x:{$x},y:{$y}}','in the room')");
		}
